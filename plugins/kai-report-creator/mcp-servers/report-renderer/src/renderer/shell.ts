...
export interface ShellOptions {
  ...
  bodyContent: string;
  tocItems: Array<{ slug: string; text: string; level: number }>;
  author: string;
  date: string;
  abstract: string;
  version: string;
...
export function buildHtmlShell(opts: ShellOptions): string {
  ...
  const fmForJsonLd: IRFrontmatter = opts.frontmatter ?? {
    title: opts.title,
    theme: opts.theme,
    date: opts.date,
...
function buildTocToggle(lang: string): string {
...
function buildTocSidebar(items: ShellOptions['tocItems'], lang: string): string {
...
function buildExportMenu(lang: string): string {
  const zh = lang === 'zh';
...
function buildSummaryCard(): string {
...
function buildScripts(opts: ShellOptions): string {
  const parts: string[] = [];
  if (opts.animations) parts.push(buildAnimationScript());
  if (opts.toc) parts.push(buildTocScript());
...
function buildAnimationScript(): string {
  return `    <script>
    (function(){
...
function buildTocScript(): string {
  return `    <script>
    (function(){
...
function buildSummaryCardScript(): string {
  return `    <script>
    (function(){
