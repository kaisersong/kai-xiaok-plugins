 ...
 *       trend: +10%
 */
export function renderKpi(block: IRBlock, options: RenderOptions): string {
  const items = parseKpiItems(block.body);
  if (items.length === 0) return '<!-- empty kpi -->';
...
function parseKpiItems(body: string): KpiItem[] {
  const items: KpiItem[] = [];
  const lines = body.split('\n');

  let current: Partial<KpiItem> | null = null;
  for (const line of lines) {
    const trimmed = line.trim();
    if (trimmed.startsWith('- label:') || trimmed.startsWith('-label:')) {
      if (current && current.label) items.push(current as KpiItem);
      current = { label: extractValue(trimmed, 'label') };
    } else if (current) {
      if (trimmed.startsWith('value:')) current.value = extractValue(trimmed, 'value');
      else if (trimmed.startsWith('trend:')) current.trend = extractValue(trimmed, 'trend');
      else if (trimmed.startsWith('status:')) current.status = extractValue(trimmed, 'status');
    }
  }
  if (current && current.label) items.push(current as KpiItem);
...
function renderTrend(trend: string): string {
