 ...
 */
export function renderDiagram(block, options) {
...
function renderSequenceDiagram(body) {
    ...
    for (const line of lines) {
        ...
        if (t.startsWith('- ')) {
            ...
            if (fromM)
                current.from = fromM[1];
            if (toM)
                current.to = toM[1];
            if (msgM)
                current.msg = msgM[1].trim().replace(/,\s*$/, '');
        }
        else if (current) {
            ...
            if (fromM)
                current.from = fromM[1].trim();
            if (toM)
                current.to = toM[1].trim();
            if (msgM)
                current.msg = msgM[1].trim();
        }
    }
    flushStep();
    if (actors.length === 0)
        return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 50"><text x="10" y="25">No actors</text></svg>';
...
function renderFlowchartDiagram(body) {
    const lines = body.trim().split('\n');
    const nodes = [];
    const edges = [];
    let section = 'none';
        ...
        if (section === 'nodes') {
            if (t.startsWith('- ')) {
                ...
                if (idM)
                    curNode.id = idM[1];
                if (kindM)
                    curNode.kind = kindM[1];
                if (labelM)
                    curNode.label = labelM[1].trim().replace(/,\s*$/, '');
            }
            else if (curNode) {
                ...
                if (idM)
                    curNode.id = idM[1].trim();
                if (kindM)
                    curNode.kind = kindM[1].trim();
                if (labelM)
                    curNode.label = labelM[1].trim();
            }
        }
        if (section === 'edges') {
            if (t.startsWith('- ')) {
                ...
                if (toM)
                    curEdge.to = toM[1];
                if (labelM)
                    curEdge.label = labelM[1].trim().replace(/,\s*$/, '');
            }
            else if (curEdge) {
                const fromM = t.match(/^from:\s*(.+?)(?:,|$)/);
                const toM = t.match(/^to:\s*(.+?)(?:,|$)/);
                const labelM = t.match(/^label:\s*(.+?)(?:,|$)/);
                if (fromM)
                    curEdge.from = fromM[1].trim();
                if (toM)
                    curEdge.to = toM[1].trim();
                if (labelM)
                    curEdge.label = labelM[1].trim();
            }
        }
    }
    flushNode();
...
function renderTreeDiagram(body) {
...
function renderMindmapDiagram(body) {
    const lines = body.trim().split('\n');
    let center = '';
    const branches = [];
    let currentBranch = null;
    for (const line of lines) {
        ...
        if (t.startsWith('items:') && currentBranch)
            continue;
