 ...
 */
export function renderChart(block, options) {
...
function buildEchartsOption(type, body, opts) {
...
function parseChartBody(body) {
    ...
    for (const line of lines) {
        ...
        if (inDatasets) {
            ...
            if (nameMatch) {
                if (currentDataset)
                    result.datasets.push(currentDataset);
        ...
        if (inNodes) {
            ...
            if (nodeMatch)
                currentNodes.push({ name: nodeMatch[1].trim() });
        }
        if (inLinks) {
    ...
    if (currentLinks.length > 0)
        result.links = currentLinks;
    return result;
}
function buildAxisChart(type, data, _opts) {
...
function buildPieChart(data, _opts) {
...
function buildRadarChart(data, _opts) {
...
function buildScatterChart(data, _opts) {
...
function buildFunnelChart(data, _opts) {
    return {
        tooltip: { trigger: 'item' },
        series: [{
                type: 'funnel',
...
function buildSankeyChart(data, _opts) {
    return {
        tooltip: { trigger: 'item' },
        series: [{
                type: 'sankey',
