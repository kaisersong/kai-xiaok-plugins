...
const server = new McpServer({ name: 'report-renderer', version: '2.0.0' }, { capabilities: { tools: {} } });
// Tool: validate_ir
server.tool('validate_ir', 'Validate a .report.md IR file for syntax and semantic correctness', { ir_content: z.string().describe('Complete .report.md IR content') }, async ({ ir_content }) => {
    const result = handleValidateIR({ ir_content });
    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
});
// Tool: list_themes
server.tool('list_themes', 'List all available report themes with descriptions', {}, async () => {
    const result = handleListThemes();
    return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
});
// Tool: render_report
server.tool('render_report', 'Render a .report.md IR into a complete HTML file', {
...
server.tool('preview_section', 'Render a single section IR fragment into HTML', {
