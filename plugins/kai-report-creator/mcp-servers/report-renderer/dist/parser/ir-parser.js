 ...
 */
export function parseSections(body, blocks, lineOffset = 0) {
    const lines = body.split('\n');
    const sections = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (headingMatch) {
            ...
            if (currentSection) {
                ...
                currentSection.blocks = blocks.filter(b => b.lineStart >= currentSection.lineStart && b.lineEnd <= i + lineOffset);
    ...
    if (currentSection) {
        currentSection.content = contentLines.join('\n');
        currentSection.blocks = blocks.filter(b => b.lineStart >= currentSection.lineStart);
        sections.push(currentSection);
    }
    return sections;
}
...
//# sourceMappingURL=ir-parser.js.map